...
<h3 class="page-title">تعديل نظام تسليم التبريد </h3>

<form action="{{ route('systems.update', $system->id) }}" method="POST" enctype="multipart/form-data">
    @csrf {{ method_field('PUT') }}
    <div class="form-group">
        <label>التفاصيل</label>
        <textarea name="ar_detail" class="form-control ck_editor">{{$system->ar_detail}}</textarea>
    </div>

    <div class="form-group">
        <label>Detail</label>
        <textarea name="en_detail" class="form-control ck_editor">{{$system->en_detail}}</textarea>
    </div>
