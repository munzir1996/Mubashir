...
<h3 class="page-title">تعديل مبيعات </h3>

<form action="{{ route('sales.update', $sale->id) }}" method="POST" enctype="multipart/form-data">
    @csrf {{ method_field('PUT') }}
    <div class="form-group">
        <label>التفاصيل</label>
        <textarea name="ar_detail" class="form-control ck_editor">{{$sale->ar_detail}}</textarea>
    </div>

    <div class="form-group">
        <label>Detail</label>
        <textarea name="en_detail" class="form-control ck_editor">{{$sale->en_detail}}</textarea>
    </div>
